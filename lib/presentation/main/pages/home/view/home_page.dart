...
class _HomePageState extends State<HomePage> {
        ...
        builder: (context, snapshot) {
          return Column(
            ...
            children: [
              ...
              _getSection(AppStrings.newPlace.tr()),
              const SizedBox(
                height: AppSize.s14,
              ),
              _getBannerWidget(snapshot.data?.banners),
              _getSection(AppStrings.popurlarHotel.tr()),
              _getServicesWidget(snapshot.data?.services),
              _getSection(AppStrings.recomended.tr()),
  ...
  _getStoresWidget(List<Store>? store) {
    if (store != null) {
      return Padding(
        padding: const EdgeInsets.only(
          left: AppPadding.p12,
          ...
          children: [
            GridView.count(
              ...
              children: List.generate(store.length, (index) {
                return InkWell(
                  onTap: () {
                    Navigator.pushNamed(context, Routes.storeDetailRoute);
                  },
                  child: Card(
                    elevation: AppSize.s4,
                    child: Image.network(
